...
struct SignupStep: Identifiable, Sendable {
    let id: Int
    let title: String
    let icon: String
...
struct SignupFunnelView: View {
    ...
    private func handleSubmit() {
        ...
        analytics.identify(userId: UUID().uuidString, traits: [
            ...
            "plan": AnyCodable(selectedPlan.rawValue)
        ])
    ...
    private func trackStepViewed() {
        analytics.track(event: "Step Viewed", properties: ["step": AnyCodable(currentStep)])
    }
}
        ...
        VStack(spacing: 20) {
            ...
            VStack(spacing: 12) {
                ForEach(Plan.allCases) { plan in
                    Button {
                        selectedPlan = plan
                        analytics.track(event: "Plan Selected", properties: ["plan": AnyCodable(plan.rawValue)])
                    } label: {
                        HStack {
            ...
            .onChange(of: agreeToTerms) { newValue in
                analytics.track(
                    ...
                    properties: ["agreed": AnyCodable(newValue), "step": 3]
                )
            }
